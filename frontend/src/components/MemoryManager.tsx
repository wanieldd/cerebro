      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div>
          ...
          <div className="bg-warm-surface border border-warm-border rounded-xl p-4">
            <div className="flex items-start gap-3">
              <Brain size={20} className="text-mustard shrink-0 mt-0.5" />
              <div>
        ...
        <div className="bg-warm-surface border border-warm-border rounded-xl p-4 space-y-3">
          <div className="text-sm text-warm-text font-medium flex items-center gap-2">
            <Plus size={16} className="text-mustard" />
            Add a memory
          </div>
          <div className="space-y-2">
            <div>
              ...
              <input
                ...
                className="w-full bg-warm-bg border border-warm-border rounded-lg px-3 py-2 text-warm-text placeholder-warm-muted focus:outline-none focus:ring-2 focus:ring-mustard text-sm"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-warm-bg border border-warm-border rounded-lg px-3 py-2 text-warm-text placeholder-warm-muted focus:outline-none focus:ring-2 focus:ring-mustard text-sm resize-none"
              />
            </div>
          </div>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-mustard text-black rounded-lg hover:opacity-90 transition-colors text-sm disabled:opacity-50 font-medium"
          >
        ...
        {!loading && memories.length === 0 && (
          <div className="bg-warm-surface border border-warm-border rounded-xl p-4">
            <div className="text-sm text-warm-text font-medium flex items-center gap-2 mb-3">
              <Lightbulb size={16} className="text-mustard" />
              Try these examples
            </div>
            <div className="space-y-2">
              {EXAMPLES.map((ex) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="text-xs text-mustard font-mono">{ex.key}</div>
                    <div className="text-xs text-warm-text mt-0.5 truncate">{ex.content}</div>
                  </div>
                  <button
                    onClick={() => handleUseExample(ex)}
                    className="px-2 py-1 text-xs text-warm-muted hover:text-mustard transition-colors shrink-0 ml-2"
                  >
                    Use this
                  </button>
                </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <code className="text-xs text-mustard font-mono bg-warm-elevated px-1.5 py-0.5 rounded">{mem.key}</code>
                      <button
